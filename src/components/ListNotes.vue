...
const routes: Notes[] = router.getRoutes()
...
const notes = computed(() =>
  [...(props.notes || routes), ...props.extra || []]
    .sort((a, b) => +new Date(b.date) - +new Date(a.date))
)

const getYear = (a: Date | string | number) => new Date(a).getFullYear()
const isSameYear = (a?: Date | string | number, b?: Date | string | number) => a && b && getYear(a) === getYear(b)
function isSameGroup(a: Notes, b?: Notes) {
  return (isSameYear(a.date, b?.date))
}

function getGroupName(p: Notes) {
...
<template>
  <ul>
    ...
    <template v-for="route, idx in notes" :key="route.path">
      <div
        v-if="!isSameGroup(route, notes[idx - 1])"
